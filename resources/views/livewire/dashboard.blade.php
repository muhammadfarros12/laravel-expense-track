   ...
   <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
       ...
       <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
           ...
           <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-purple-500">
               ...
               <div class="text-3xl font-bold text-gray-800">Rp{{ number_format($totalSpent, 0) }}</div>
               @if($monthlyBudget > 0)
                   <div class="mt-2 text-sm {{ $totalSpent > $monthlyBudget ? 'text-red-600' : 'text-green-600' }}">
                       {{ $totalSpent > $monthlyBudget ? 'Over' : 'Under' }} budget by Rp{{ number_format(abs($monthlyBudget - $totalSpent), 0) }}
                   </div>
               @endif
           </div>
           ...
           <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-blue-500">
               ...
               <div class="text-3xl font-bold text-gray-800">Rp{{ number_format($monthlyBudget, 0) }}</div>
               @if($monthlyBudget > 0)
                   <div class="mt-3">
                       <div class="flex items-center justify-between text-xs text-gray-600 mb-1">
                           <span>{{ $percentageUsed }}% used</span>
                           <span>Rp{{ number_format($monthlyBudget - $totalSpent, 0) }} left</span>
                       </div>
                   ...
                   @forelse($topCategories as $category)
                       <div class="flex items-center justify-between">
                           ...
                           <div class="text-right">
                               <div class="font-bold text-gray-800">Rp{{ number_format($category->total, 0) }}</div>
                           </div>
                       </div>
                   @empty
               ...
               <div class="space-y-3">
                   @forelse($recentExpenses as $expense)
                       <div class="flex items-center justify-between py-3 border-b border-gray-100 last:border-0">
                           ...
                           <div class="text-right">
                               <div class="font-bold text-gray-800">-Rp{{ number_format($expense->amount, 0) }}</div>
